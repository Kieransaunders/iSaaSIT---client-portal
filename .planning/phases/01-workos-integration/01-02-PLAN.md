...
1. Add state for billing email:
   ```typescript
   const [billingEmail, setBillingEmail] = useState('');
   ```

2. Pre-populate billing email from user.email if available:
   ```typescript
   // In component, after getting user from loader data
   const [billingEmail, setBillingEmail] = useState(user.email || '');
   ```

3. Add billing email input field after org name field:
   - Label: "Billing Email"
   - Input type="email", required
   - Placeholder: "billing@yourcompany.com"
   - Helper text: "Invoices and billing notifications will be sent here"

4. Update form validation to require both fields:
...
- Form has two required fields visible
  ...
  <done>
- Billing email state exists and is pre-populated from user.email
...
- Billing email pre-populated from user.email
