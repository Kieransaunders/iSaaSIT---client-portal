...
4. **Available Plans section (3-column grid):**
...
5. **Cancel subscription section (at bottom, only if subscriptionStatus is "active"):**
...
- Use existing Card, Button, Badge components
  ...
  <verify>
Run `npx tsc --noEmit` to check for type errors. Verify billing page imports useQuery for billing queries. Verify UpgradeButton component is used with variant IDs. Verify cancel flow uses AlertDialog with confirmation.
  </verify>
  <done>
Billing page shows real plan name and status, live usage progress bars with colors, upgrade buttons that open Lemon Squeezy overlay, plan comparison cards, and cancel subscription flow with confirmation dialog.
  </done>
</task>
...
4. Click "Upgrade" on a plan card — verify Lemon Squeezy checkout overlay appears in-app
